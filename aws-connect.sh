...
VPN_HOST="${VPN_HOST:-"cvpn-endpoint-<id>.prod.clientvpn.us-east-1.amazonaws.com"}"
# path to the patched openvpn
OVPN_BIN="${OVPN_BIN:-"./openvpn"}"
# path to the configuration file
OVPN_CONF="${OVPN_CONF:-"vpn.conf"}"
PORT="${PORT:-"1194"}"
PROTO="${PROTO:-"udp"}"
...
Darwin*) open "$URL" ;;
*)
  echo "Could not determine 'open' command for this OS"
  exit 1
  ;;
esac
